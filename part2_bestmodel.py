...
# ---------- Custom transformer (canonical name used in both parts) ----------
class OutlierScoreAppender(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if self.method == "iforest":
            self.model_ = IsolationForest(contamination=self.contamination, n_estimators=200,
    ...
    def transform(self, X):
        ...
        X2 = X.copy(); X2["outlier_score"] = score
        return X2
...
def build_pre_from_cfg(cfg, X):
    # Rebuild the transformers-only preprocessor from JSON if joblib load fails
...
# ---------- Load preprocessor (or rebuild from JSON) ----------
try:
    pre = load("best_preprocess.joblib")     # works if class/module matches
except Exception as e:
    print(f"[WARN] joblib load failed ({e}). Rebuilding preprocessor from JSON...")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        test_probs = final_pipe.predict_proba(test_df)[:,1]
        test_preds = (test_probs >= best_tau).astype(int)
        pd.DataFrame({"Pred": test_preds}).to_csv("predictions_test.csv", index=False)
